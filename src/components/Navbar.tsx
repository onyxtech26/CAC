      ...
      >
          ...
          {navLinks.map((link) => (
            ...
            >
              ...
              {activeSection === link.id && (
                <span className="absolute bottom-0 left-0 w-full h-[1px] bg-secondary animate-pulse" />
              )}
            </a>
          ))}
        </div>
        ...
        <div className="hidden sm:flex items-center gap-4 h-full py-2">
          <button
            ...
            className="bg-secondary text-white px-6 h-full rounded-full font-mono text-xs uppercase font-semibold transition-all hover:bg-tertiary hover:border-tertiary active:scale-95 duration-200 border border-secondary shadow-md hover:shadow-tertiary/20"
          >
            Book Consultation
          </button>
        </div>
