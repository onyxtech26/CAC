...
import { Counter, Heading, Reveal, Tag } from "./ui";
import ContactForm from "./ContactForm";
import {
  CASE_STUDIES, CONTACT, HERO_STATS, PRINCIPLES_HOME, PROCESS,
  SERVICES, TAGLINES, TESTIMONIALS, TRACK_RECORD, waLink,
} from "../data";

const DEFAULT_FEATURED_INTRO: ReactNode = (
  <>
    Every engagement draws on a wider catalogue of{" "}
    <Link to="/services" className="text-gold-2 underline decoration-gold-2/40 underline-offset-4">twelve forensic capabilities</Link>{" "}
    — these three anchor most estate and ownership recoveries.
  </>
);
...
export function FeaturedDisciplines({
  eyebrow = "What we uncover",
  title = <>Three disciplines.<br />One unbroken chain of evidence.</>,
  intro = DEFAULT_FEATURED_INTRO,
  noIntro = false,
}: { eyebrow?: string; title?: ReactNode; intro?: ReactNode; noIntro?: boolean }) {
    ...
    <section className="relative py-24">
      <div className="mx-auto max-w-[1320px] px-5 lg:px-8">
        <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end">
          ...
          {!noIntro && <Reveal delay={120}><p className="max-w-md text-sm text-ivory/65">{intro}</p></Reveal>}
        </div>
...
/* ============== FIRM SUMMARY ============== */
export function FirmSummary({
  eyebrow = "The firm",
  title = <>Forensic rigour,<br />delivered with discretion.</>,
  italic = TAGLINES[0],
}: { eyebrow?: string; title?: ReactNode; italic?: string }) {
  return (
    <section className="relative overflow-hidden py-24">
      ...
      <div className="relative mx-auto grid max-w-[1320px] grid-cols-1 items-center gap-12 px-5 lg:grid-cols-2 lg:px-8">
        ...
        <div>
          <Heading eyebrow={eyebrow} title={title} italic={italic} />
          <Reveal delay={120}>
...
export function ProcessTeaser({
  eyebrow = "Methodology · abridged",
  title = <>A standardized chain of<br />investigation, end to end.</>,
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-5">
          {PROCESS.slice(0, 5).map((p, i) => (
            <Reveal key={p.no} delay={i * 90}>
              <div className="plate plate-hover group relative flex h-full flex-col overflow-hidden rounded-md">
                <div className="relative h-32 w-full overflow-hidden bg-navy-3">
                  <img src={p.img} alt={p.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-navy-2 via-navy-2/40 to-transparent" />
                  <span className="absolute top-2 left-2 font-display text-2xl font-bold text-gold-gradient bg-navy/80 px-2 py-0.5 rounded border border-gold-2/30 backdrop-blur">
                    {p.no}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-4">
                  <p className="font-display text-base font-semibold leading-tight text-ivory">{p.title}</p>
                  <p className="mt-1 font-mono text-[9px] uppercase tracking-wide-2 text-gold-2/80">{p.sub}</p>
                  <p className="mt-2.5 flex-1 text-xs leading-relaxed text-ivory/65">{p.details}</p>
                </div>
              </div>
            </Reveal>
          ))}
...
export function TrackRecord({
  ...
  title = <>Closed engagements,<br />documented outcomes.</>,
  note = "227+ cases completed · 30+ active · 17+ years on the record",
}: { eyebrow?: string; title?: ReactNode; note?: string }) {
  return (
    <section className="relative overflow-hidden border-y border-gold-2/10 bg-ink py-24">
      ...
      <div className="relative mx-auto max-w-[1320px] px-5 lg:px-8">
        <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end">
          <Heading eyebrow={eyebrow} title={title} />
          <Reveal delay={100}><p className="max-w-sm font-mono text-[11px] uppercase tracking-wide-2 text-mute">{note}</p></Reveal>
        </div>
          ...
          {TRACK_RECORD.map((t, i) => (
            <Reveal key={t.code} delay={i * 70}>
              <div className="group relative h-full bg-navy-2/70 p-7 transition hover:bg-navy-3/70">
                ...
                <p className="mt-4 font-display text-5xl text-gold-gradient"><Counter to={t.count} suffix={t.suffix} /></p>
            ...
            <Reveal key={c.title} delay={i * 80} className="snap-start">
              <article className="plate plate-hover group relative flex h-full w-[300px] flex-col overflow-hidden rounded-lg sm:w-[330px]">
                <div className={`relative h-52 overflow-hidden bg-gradient-to-br ${c.tone}`}>
                  <img src={c.img} alt={c.title} loading="lazy" className="absolute inset-0 h-full w-full object-cover transition duration-[1200ms] ease-out group-hover:scale-[1.07]" />
                  ...
                  {c.meta && <span className="absolute bottom-3 left-3 font-mono text-[9px] uppercase tracking-wide-2 text-ivory/70">{c.meta}</span>}
                </div>
  ...
  return (
    <section className="relative overflow-hidden py-24">
      ...
      <div className="relative mx-auto max-w-[1100px] px-5 text-center lg:px-8">
        <Heading className="mx-auto" eyebrow={eyebrow} title={title} />
        <Reveal>
...
export function ContactTeaser({
  eyebrow = "Open a case file",
  title = <>Speak with a senior<br />forensic consultant.</>,
  italic = TAGLINES[0],
}: { eyebrow?: string; title?: ReactNode; italic?: string }) {
  return (
    <section className="relative py-24">
            ...
            <div className="mt-8 space-y-4">
              {[
                ...
                <div key={r.t} className="flex items-start gap-4 rounded-md border border-gold-2/15 bg-navy-2/40 p-4">
                  <span className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full border border-gold-2/30 text-gold-2">
                    {r.g === "users" ? (
                      <img src="/assets/mohaan-profile.png" alt={r.s} className="h-full w-full object-cover" />
                    ) : (
                      <Icon name={r.g} size={18} />
                    )}
                  </span>
                  <div>
